...
class MethodologyValidator:
        ...
        try:
            ...
            try:
                ...
                if e.instance is not None:
                    errors.append(
                        f"  Problem value: {e.instance} (type: {type(e.instance).__name__})"
                    )
    ...
    def _validate_tree_depth_consistency(
        self, methodology: Dict, errors: List[str]
    ) -> None:
        ...
        if len(unique_depths) > 1:
            errors.append(
                f"Inconsistent tree depth: found depths {unique_depths}. "
    ...
    def _validate_action_coverage(self, methodology: Dict, errors: List[str]) -> None:
        ...
        coverage_percentage = (
            (len(covered_paths) / total_combinations) * 100
        ...
        if coverage_percentage < 25:
            print(
                f"⚠️  Warning: Very low decision coverage ({coverage_percentage:.1f}%) in {methodology['name']}. "
    ...
    def _get_covered_paths(
        self, node: Dict, current_path: List[str] = None
    ) -> List[List[str]]:
            ...
            for param_name, mapping in vector_meta["parameterMappings"].items():
                if "enumType" in mapping:
                    ...
                    if enum_type not in declared_enums:
                        errors.append(
                            f"Vector metadata parameter '{param_name}' references "
                    ...
                    if "valueMappings" in mapping:
                        ...
                        if unmapped:
                            errors.append(
                                f"Vector metadata parameter '{param_name}' missing "
                        ...
                        if invalid_mappings:
                            errors.append(
                                f"Vector metadata parameter '{param_name}' has "

                ...
                try:
                    ...
                    for attr_name in dir(module):
                        ...
                        if (isinstance(attr, type) and 
                            attr_name.startswith('Decision') and
                            hasattr(attr, '__init__') and
                            not attr_name.endswith('Level') and
                            not attr_name.endswith('Status')):
...
def list_methodologies() -> Dict[str, str]:
    ...
    return {name: plugin.description for name, plugin in registry.list_plugins().items()}
...
try:
    from ssvc.legacy import (
        ExploitationLevel, Automatable, TechnicalImpact, MissionWellbeingImpact,
        DecisionPriority, ActionCISA, OutcomeCISA, Methodology
    )
except ImportError:
